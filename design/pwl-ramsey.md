...
lemma pigeonhole_edges (edge : Sym2 (Fin 6) → Color)
  ...
  | ($v, $x) → $c :: $edge' =>
    ...
    | (#v, $y) → #c :: $edge'' =>
      match edge'' as multiset (Sym2 (Fin 6) → Color) with
      | (#v, $z) → #c :: _ => exact ⟨v, x, c, y, z⟩
      | (#v, $x') → $c' :: (#v, $y') → #c' :: (#v, $z') → #c' :: _ => exact ⟨v, x', c', y', z'⟩
    | (#v, $x') → (!#c & $c') :: (#v, $y') → #c' :: (#v, $z') → #c' :: _ => exact ⟨v, x', c', y', z'⟩
...
例えば、最初のケース `(#x, #y) → #c` では：
- 外側のマッチから: `edge ⟦(v,x)⟧ = c`, `edge ⟦(v,y)⟧ = c`, `v ≠ x`, `v ≠ y`, ...
- 内側のマッチから: `edge ⟦(x,y)⟧ = c`

これらを合わせると、定理のパターンが要求する関係がすべて充足されることが自動で確認される。証明者は変数の対応を指示するだけでよく、関係の証明を明示的に構築する必要がない。
...
| 存在量化 | 明示的に `∃` | パターン変数に吸収 |
| 行数（主定理の証明） | 約25行 | 約13行 |
| 補助補題 | 1つ（鳩の巣原理、約10行） | 2つ（鳩の巣原理 約13行、2色網羅性 約4行） |
| 鳩の巣原理の証明 | `by_contra` + `omega` | ネストされた multiset マッチ |
| `obtain`（3頂点の取り出し） | 1箇所 | 0箇所（multiset `::` に吸収） |
| `have`（辺の色の証明） | 3箇所 | 0箇所（非線形パターンに吸収） |
| 辺の色の場合分け | `rcases` フラット4ケース | `match` フラット4ケース |
| 反対色の導出 | `cases ... <;> simp_all` 3箇所 | multiset マッチ + `!#c & $c'` で自動 |
| 網羅性の証明 | 不要（`rcases` は構造的に網羅的） | 主定理: 1箇所（`two_color_exhaustive`）、`pigeonhole_edges`: 各レベルで自明 |
...
パターンマッチ指向版は主定理が約13行、補助補題 `pigeonhole_edges` が約13行、
`two_color_exhaustive` が約4行で、合計約30行。
`monochromatic` や `same_color_neighbors` の定義は不要。
`pigeonhole_edges` はパターンマッチ指向版ではネストされた multiset マッチで
構造的に証明されており、各レベルの網羅性は Color の場合分けまたは排中律に帰着するため自明である。
`two_color_exhaustive` は `decide` で閉じる1行の補題である。
...
- **パターンマッチ指向版**: 各ケースではパターン変数の値を列挙するだけでよい。
